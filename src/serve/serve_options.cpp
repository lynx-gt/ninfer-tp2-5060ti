...
std::string serve_usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "across two GPUs and requires --devices; it supports --spec mtp, --spec dflash2, "
           "and --vision.\n"
...
ServeOptions parse_serve_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--host") {
            ...
            options.vision_max_merged_tokens = static_cast<std::uint32_t>(
                parse_nonnegative_int(require_value("--vision-max-merged-tokens"),
                                      "vision-max-merged-tokens"));
