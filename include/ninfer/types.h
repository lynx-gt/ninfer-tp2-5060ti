...
struct EngineOptions {
    ...
    // `qwen3.8-27b`) with `SpeculativeBackend::None` or `Mtp`; `qwen3.6-35b-a3b` and
    // `enable_vision` are rejected at construction, and the DFlash/DFlash2 backends are rejected
    // until their own split path exists. `tp == 1` is bit-identical to the single-device path.
    int tp = 1;
